...
import json #allows code to save and load stats to/from a file 
import os #allows code to check if stats file exists 
...
#Assign states file name 

#stats_file = "game_stats.json"
stats_file = os.path.join(os.path.expanduser("~"), "Desktop", "game_stats.json")

#--------------------------------------------------------------------------------------------------------------

#Create game functions 

#create a function that loads stats from file or creates blank stats if the file not found 
def load_stats(): 
    if os.path.exists(stats_file): #checks if stats file already exists 
        with open(stats_file, "r") as f: #opens file in read mode, assign alias 'f' 
            return json.load(f) #loads and sends existing stats to game loop 
    return { #returns blank stats if no file found 
        "games_played": 0, 
        "levels_passed": 0, 
        "total_attempts": 0, 
        "total_time": 0, 
        "level_details": [] #stores a list of dictionaries for each level played 
    }

#create a function that automatically saves stats to file 
def save_stats(game_stats):
    with open(stats_file, "w") as f: #opens file in write mode & creates if it doesn't exist, assign alias 'f' 
        json.dump(game_stats, f, indent=4) #converting game stats to json file 


#create a function that prints game stats summary at the end of the game 
def print_stats(game_stats, session_levels): 
    print("\n------------------------------------------------ 📊 Game Statistics ------------------------------------------------\n")
    print(f"Total games played: {game_stats['games_played']}")
    print(f"Total levels passed: {game_stats['levels_passed']}")
    print (f"Total attempts across all games: {game_stats['total_attempts']}")
    print(f"Total time across all games: {round(game_stats['total_time'], 1)}")
    print("-- This game's level breakdown --")
    for level in session_levels: #shows current session level details 
        if level["complete"]: 
            status = "✅" 
        else: 
            status = "❌"
        #takes appended stats from 'level_details' list in 'game_stats', referencing each level(dictionary) in the list 
        print (f"{status} Level {level['level_num']} ({level['level_name']}): {level['attempts']} attempts | {level['time_taken']}s.") 
...
#load stats automatically when game starts 

#create variable to hold returned value of load_stats to be accessed and updated by rest of code
game_stats = load_stats() 
game_stats["games_played"] += 1 #increments games played count each time script is run 
save_stats(game_stats) #passes game_stats through save_states function to record a new game session
...
session_levels = [] #tracks only current session's level details 
...
for level_num, level in enumerate(levels, start=1): #enumerate creates index number for each level, level_num = index, level = dictionary 
    ...
    print(f"\n--- Level {level_num}: {level_name} | Maximum attempts: {max_attempts} ---\n") #\n to create space between next line of text

#--------------------------------------------------------------------------------------------------------------

    failed_level = True #initially set to true to enter while loop, break out when level passed
    ...
    while failed_level: 
        ...
        while position < 100: 
            ...
            if attempts >= max_attempts: #checks if max attempts reached, ignores if false 
                ...
                print(f"👾 Failed to complete Level {level_num}: {level_name} | Try Again!\n")

                #record failed level stats and saves automatically 
                ...
                level_entry = {
                    ...
                    "complete": False #level was failed 
                }
                game_stats["level_details"].append(level_entry)
                session_levels.append(level_entry)
                save_stats(game_stats)

                failed_level = True #level failed, replays level 
                break #exits level loop when atempts exceeded     
    ...
    level_entry = {
        ...
        "attempts": attempts, 
        "time_taken": time_taken, 
         "complete": True #level was failed 
        }
    game_stats["level_details"].append(level_entry)
    session_levels.append(level_entry)
    save_stats(game_stats)
...
#Print stats summary at the end of the game 

print_stats(game_stats, session_levels)